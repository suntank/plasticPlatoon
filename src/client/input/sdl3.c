...
static SDL_Gamepad *registered_controllers[SS_MAX_LOCAL_PLAYERS];
	...
	while (SDL_PollEvent(&event))
	{
		...
		switch (event.type)
		{
			...
			case SDL_EVENT_GAMEPAD_BUTTON_DOWN :
			{
				...
				int gamepad_index = IN_FindGamepadIndexByInstanceId(event.gbutton.which);
			...
			case SDL_EVENT_GAMEPAD_AXIS_MOTION :  /* Handle Controller Motion */
			{
				...
				int gamepad_index = IN_FindGamepadIndexByInstanceId(event.gaxis.which);
			...
			case SDL_EVENT_GAMEPAD_REMOVED :
				if (IN_FindGamepadIndexByInstanceId(event.gdevice.which) >= 0) {
...
IN_Controller_Init(qboolean notify_user)
{
	...
	for (int i = 0; i < numjoysticks; i++)
	{
		...
		{
			SDL_Gamepad *opened_controller = SDL_OpenGamepad(joysticks[i]);
			...
			registered_controller_ids[registered_controller_count] =
				SDL_GetJoystickID(SDL_GetGamepadJoystick(opened_controller));
			...
			Com_Printf("Enabled as Gamepad, settings:\n%s\n",
				   SDL_GetGamepadMapping(opened_controller));

#ifndef NO_SDL_GYRO

			const qboolean found_gyro =
				(SDL_GamepadHasSensor(opened_controller, SDL_SENSOR_GYRO)
				&& SDL_SetGamepadSensorEnabled(
					opened_controller, SDL_SENSOR_GYRO, true));

			const qboolean found_accel =
				(SDL_GamepadHasSensor(opened_controller, SDL_SENSOR_ACCEL)
				&& SDL_SetGamepadSensorEnabled(
					opened_controller, SDL_SENSOR_ACCEL, true));

			if ((opened_controller == controller) && found_gyro && found_accel)
			{
				show_gyro = true;
				Com_Printf("Sensors enabled: Gyro at %.2f Hz, Accelerometer at %.2f Hz\n",
					SDL_GetGamepadSensorDataRate(opened_controller, SDL_SENSOR_GYRO),
					SDL_GetGamepadSensorDataRate(opened_controller, SDL_SENSOR_ACCEL));
			}
			else if (opened_controller == controller)
			{
				...
				SDL_SetGamepadSensorEnabled(opened_controller, SDL_SENSOR_GYRO, false);
				SDL_SetGamepadSensorEnabled(opened_controller, SDL_SENSOR_ACCEL, false);
			}

			bool hasLED = SDL_GetBooleanProperty(SDL_GetGamepadProperties(opened_controller),
				SDL_PROP_JOYSTICK_CAP_RGB_LED_BOOLEAN, false);
			if ((opened_controller == controller) && hasLED)
			{
				SDL_SetGamepadLED(opened_controller, 0, 80, 0);	// green light
			}
			...
			{
				joystick_haptic = SDL_OpenHapticFromJoystick(SDL_GetGamepadJoystick(opened_controller));
			}
			...
			bool hasRumble = SDL_GetBooleanProperty(SDL_GetGamepadProperties(opened_controller),
				SDL_PROP_GAMEPAD_CAP_RUMBLE_BOOLEAN, false);
			if ((opened_controller == controller) && hasRumble)
			{
				show_haptic = true;
...
IN_Controller_Shutdown(qboolean notify_user)
{
	...
	for (i = 0; i < registered_controller_count; ++i)
	{
		...
		{
			SDL_CloseGamepad(registered_controllers[i]);
