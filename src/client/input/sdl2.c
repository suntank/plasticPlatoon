...
static SDL_GameController *registered_controllers[SS_MAX_LOCAL_PLAYERS];
static SDL_JoystickID registered_controller_ids[SS_MAX_LOCAL_PLAYERS];
static char registered_controller_names[SS_MAX_LOCAL_PLAYERS][32];
static int registered_controller_count = 0;
...
static void
IN_DispatchKeyEvent(int keynum, qboolean down, qboolean special, int gamepad_index)
{
	Key_SetEventGamepadIndex(gamepad_index);
	Key_Event(keynum, down, special);
}

static int
IN_FindGamepadIndexByInstanceId(SDL_JoystickID id)
{
	int i;

	for (i = 0; i < registered_controller_count; ++i)
	{
		if (registered_controller_ids[i] == id)
		{
			return i;
		}
	}
...
int
IN_GetDetectedGamepadCount(void)
{
	return registered_controller_count;
}

const char *
IN_GetDetectedGamepadName(int index)
{
	if (index < 0 || index >= registered_controller_count)
	{
		return "Unavailable";
	}

	return registered_controller_names[index];
}
...
ImuJoy_PrepareSamples(void)
{
	if (num_accel[0] && num_accel[1] && num_accel[2])
	{
		...
		// Now the data can be handled like SDL_CONTROLLERSENSORUPDATE.
	...
	int gamepad_index)
{
	if (new_state != *state_store)
	{
		*state_store = new_state;
		IN_DispatchKeyEvent(keynum, *state_store, true, gamepad_index);
	}
}
...
IN_Update(void)
{
	...
	static qboolean left_trigger[SS_MAX_LOCAL_PLAYERS] = {false};
	static qboolean right_trigger[SS_MAX_LOCAL_PLAYERS] = {false};
	static qboolean left_stick[SS_MAX_LOCAL_PLAYERS][4] = {{false}};   // left, right, up, down virtual keys
	...
	while (SDL_PollEvent(&event))
	{
		...
		{
			case SDL_MOUSEWHEEL:
				IN_DispatchKeyEvent((event.wheel.y > 0 ? K_MWHEELUP : K_MWHEELDOWN), true, true, -1);
				IN_DispatchKeyEvent((event.wheel.y > 0 ? K_MWHEELUP : K_MWHEELDOWN), false, true, -1);
				break;
			...
			case SDL_MOUSEBUTTONUP:
				...
				IN_DispatchKeyEvent(key, (event.type == SDL_MOUSEBUTTONDOWN), true, -1);
				break;
			...
			case SDL_KEYUP:
			{
				...
				if (sc >= SDL_SCANCODE_1 && sc <= SDL_SCANCODE_0)
				{
					...
					IN_DispatchKeyEvent(key, down, false, -1);
				}
				else
				{
					...
					if(sc == SDL_SCANCODE_GRAVE && kc != '\'' && kc != '"')
					{
						...
						if((event.key.keysym.mod & (KMOD_CAPS|KMOD_SHIFT|KMOD_ALT|KMOD_CTRL|KMOD_GUI)) == 0)
						{
							...
							IN_DispatchKeyEvent(K_CONSOLE, down, true, -1);
							consoleKeyCode = kc;
						}
					}
					else if ((kc >= SDLK_SPACE) && (kc < SDLK_DELETE))
					{
						IN_DispatchKeyEvent(kc, down, false, -1);
					}
					else
					{
						...
						if(key != 0)
						{
							IN_DispatchKeyEvent(key, down, true, -1);
						}
						else
						{
			...
			case SDL_CONTROLLERBUTTONDOWN:
			{
				...
				int gamepad_index = IN_FindGamepadIndexByInstanceId(event.cbutton.which);
				int q2_key = (btn == joy_escbutton) ? K_ESCAPE : K_JOY_FIRST_BTN + btn;

				if (gamepad_index > 0 && !SS_IsLobbyActive() &&
					!SS_ShouldAcceptSessionKey(gamepad_index + 2, q2_key) &&
					!SS_ShouldCaptureGameplayDevice(gamepad_index + 2))
				{
					break;
				}

				// Handle Esc button first, to override its original key
				IN_DispatchKeyEvent(q2_key, down, true, gamepad_index);
				break;
			}
			...
			case SDL_CONTROLLERAXISMOTION:  /* Handle Controller Motion */
			{
				...
				int gamepad_index = IN_FindGamepadIndexByInstanceId(event.caxis.which);
				...
				if (gamepad_index > 0 && !SS_IsLobbyActive() &&
					!SS_IsMenuInputActiveForDevice(gamepad_index + 2) &&
					!SS_ShouldCaptureGameplayDevice(gamepad_index + 2))
				{
					break;
				}
				...
				switch (event.caxis.axis)
				{
					case SDL_CONTROLLER_AXIS_TRIGGERLEFT:
						IN_VirtualKeyEvent(K_TRIG_LEFT, &left_trigger[gamepad_index],
							axis_value > trig_thresh, gamepad_index);
						break;

					case SDL_CONTROLLER_AXIS_TRIGGERRIGHT:
						IN_VirtualKeyEvent(K_TRIG_RIGHT, &right_trigger[gamepad_index],
							axis_value > trig_thresh, gamepad_index);
						break;
				}

				if (SS_ShouldCaptureGameplayDevice(gamepad_index + 2) &&
					!SS_IsMenuInputActiveForDevice(gamepad_index + 2))
				{
					switch (event.caxis.axis)
					{
						case SDL_CONTROLLER_AXIS_LEFTX:
							SS_SetDeviceAxis(gamepad_index + 2, SS_AXIS_LEFT_X, axis_value);
							break;

						case SDL_CONTROLLER_AXIS_LEFTY:
							SS_SetDeviceAxis(gamepad_index + 2, SS_AXIS_LEFT_Y, axis_value);
							break;

						case SDL_CONTROLLER_AXIS_RIGHTX:
							SS_SetDeviceAxis(gamepad_index + 2, SS_AXIS_RIGHT_X, axis_value);
							break;

						case SDL_CONTROLLER_AXIS_RIGHTY:
							SS_SetDeviceAxis(gamepad_index + 2, SS_AXIS_RIGHT_Y, axis_value);
							break;
					}
					...
					SS_IsMenuInputActiveForDevice(gamepad_index + 2))
				{
					...
					{
						case SDL_CONTROLLER_AXIS_LEFTX:
							IN_VirtualKeyEvent(K_LEFTARROW, &left_stick[gamepad_index][0],
								axis_value < -16896, gamepad_index);
							IN_VirtualKeyEvent(K_RIGHTARROW, &left_stick[gamepad_index][1],
								axis_value > 16896, gamepad_index);
							break;

						case SDL_CONTROLLER_AXIS_LEFTY:
							IN_VirtualKeyEvent(K_UPARROW, &left_stick[gamepad_index][2],
								axis_value < -16896, gamepad_index);
							IN_VirtualKeyEvent(K_DOWNARROW, &left_stick[gamepad_index][3],
								axis_value > 16896, gamepad_index);
							break;
					}
				}
			...
			case SDL_CONTROLLERSENSORUPDATE:
				switch (event.csensor.sensor)
				{
					case SDL_SENSOR_ACCEL:
					{
						...
						vec3_t accel_data;
						accel_data[0] = event.csensor.data[0] / SDL_STANDARD_GRAVITY;
						accel_data[1] = event.csensor.data[1] / SDL_STANDARD_GRAVITY;
						accel_data[2] = event.csensor.data[2] / SDL_STANDARD_GRAVITY;
						IN_GYRO_AccumulateAccelSamples(accel_data);
						break;
					}
					...
					case SDL_SENSOR_GYRO:
						...
						IN_GYRO_AccumulateGyroSamples(event.csensor.data);
						break;
			...
			case SDL_JOYAXISMOTION:
				...
				{
					const int axis_value = event.caxis.value;
					switch (event.caxis.axis)
					{
			...
			case SDL_CONTROLLERDEVICEREMOVED:
				if (IN_FindGamepadIndexByInstanceId(event.cdevice.which) >= 0) {
			...
			case SDL_JOYDEVICEADDED:
				if (registered_controller_count < SS_MAX_LOCAL_PLAYERS)
				{
...
IN_Move(usercmd_t *cmd)
{
	...
	if (gyro_enabled)
	{
		...
		// SDL_JOYAXISMOTION, so combine the data now.
...
IN_Controller_Init(qboolean notify_user)
{
	...
	registered_controller_count = 0;
	memset(registered_controllers, 0, sizeof(registered_controllers));
	memset(registered_controller_ids, 0xff, sizeof(registered_controller_ids));
	memset(registered_controller_names, 0, sizeof(registered_controller_names));
	...
	for (int i = 0; i < SDL_NumJoysticks(); i++)
	{
		...
		if (is_controller && registered_controller_count < SS_MAX_LOCAL_PLAYERS)
		{
			SDL_GameController *opened_controller = SDL_GameControllerOpen(i);
			...
			registered_controllers[registered_controller_count] = opened_controller;
			registered_controller_ids[registered_controller_count] =
				SDL_JoystickInstanceID(SDL_GameControllerGetJoystick(opened_controller));
			Q_strlcpy(registered_controller_names[registered_controller_count],
				joystick_name, sizeof(registered_controller_names[registered_controller_count]));
			...
			Com_Printf("Enabled as Game Controller, settings:\n%s\n",
				SDL_GameControllerMapping(opened_controller));

#ifndef NO_SDL_GYRO

			const qboolean found_gyro =
				(SDL_GameControllerHasSensor(opened_controller, SDL_SENSOR_GYRO)
				&& !SDL_GameControllerSetSensorEnabled(
					opened_controller, SDL_SENSOR_GYRO, SDL_TRUE));

			const qboolean found_accel =
				(SDL_GameControllerHasSensor(opened_controller, SDL_SENSOR_ACCEL)
				&& !SDL_GameControllerSetSensorEnabled(
					opened_controller, SDL_SENSOR_ACCEL, SDL_TRUE));

			if (opened_controller == controller && found_gyro && found_accel)
			{
				show_gyro = true;
#if SDL_VERSION_ATLEAST(2, 0, 16)
				Com_Printf("Sensors enabled: Gyro at %.2f Hz, Accelerometer at %.2f Hz\n",
					SDL_GameControllerGetSensorDataRate(opened_controller, SDL_SENSOR_GYRO),
					SDL_GameControllerGetSensorDataRate(opened_controller, SDL_SENSOR_ACCEL));
#else
				Com_Printf("Gyro and accelerometer sensors enabled.\n");
#endif	// #if SDL_VERSION_ATLEAST(2, 0, 16)
			}
			else if (opened_controller == controller)
			{
				if (!found_gyro)
				{
				...
				SDL_GameControllerSetSensorEnabled(opened_controller, SDL_SENSOR_GYRO, SDL_FALSE);
				SDL_GameControllerSetSensorEnabled(opened_controller, SDL_SENSOR_ACCEL, SDL_FALSE);
			}

			if ((opened_controller == controller) && SDL_GameControllerHasLED(opened_controller))
			{
				SDL_GameControllerSetLED(opened_controller, 0, 80, 0);	// green light
			}

#endif	// !NO_SDL_GYRO

			if (opened_controller == controller)
			{
				joystick_haptic = SDL_HapticOpenFromJoystick(SDL_GameControllerGetJoystick(opened_controller));
			}
			...
			if ((opened_controller == controller) && SDL_GameControllerHasRumble(opened_controller))
#elif SDL_VERSION_ATLEAST(2, 0, 9)	// support for rumble
			if ((opened_controller == controller) && SDL_GameControllerRumble(opened_controller, 1, 1, 1) == 0)
...
IN_Controller_Shutdown(qboolean notify_user)
{
	...
	for (i = 0; i < registered_controller_count; ++i)
	{
		if (registered_controllers[i])
		{
			SDL_GameControllerClose(registered_controllers[i]);
			registered_controllers[i] = NULL;
		}
	}
